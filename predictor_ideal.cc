...
bool PREDICTOR::GetPrediction (UINT64 PC)
{
  ...
  if (numbranches < 1000)
  //  printf("%d,", weights_array[PC & ((1 << ADDRESSBITS)-1)][0][0]);
  for(int i = 0; i < GHL; i++)
  {
    if((ghr >> i) & 1)
    {
    //printf("%d,", weights_array[PC & ((1 << ADDRESSBITS)-1)][GA[i]][i + 1]);
...
void PREDICTOR::UpdatePredictor (UINT64 PC, OpType OPTYPE, bool resolveDir, bool predDir, UINT64 branchTarget)
{
 if(abs(output) < theta || (predDir != resolveDir)){
   ...
   for(int i = 0; i < GHL; i++){
     ...
     if((ghr >> i) & 1){
   	   increment_weights(PC & ((1 << ADDRESSBITS)-1),i + 1); //	weights_array[PC & ((1 << HISTORY_LENGTH)-1)][GA[i]][i] = weights_array[PC & ((1 << HISTORY_LENGTH)-1)][GA[i]][i] + 1;
   	 }
   	 else{
   	   decrement_weights(PC & ((1 << ADDRESSBITS)-1),i + 1);  //	weights_array[PC & ((1 << HISTORY_LENGTH)-1)][GA[i]][i] = weights_array[PC & ((1 << HISTORY_LENGTH)-1)][GA[i]][i] - 1;
   	 }
   }
...
void PREDICTOR::increment_weights(uint8_t address, uint8_t index)
{
  if (index == 0){
    ...
    if (weights_array[address][GA[index - 1]][index] < 63)
      weights_array[address][GA[index - 1]][index]++;
  }
}
...
void PREDICTOR::decrement_weights(uint8_t address, uint8_t index)
{
  if (index == 0){
    ...
    if (weights_array[address][GA[index-1]][index] > -64)
      weights_array[address][GA[index-1]][index]--;
  }
}
